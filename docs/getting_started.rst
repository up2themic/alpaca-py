...
Before installing, make sure pip itself is up to date in the same environment so the installer
can resolve dependencies correctly.
...
If you skipped the pip upgrade above or still encounter installation errors, upgrade pip in the
same Python environment to avoid mixing versions across environments.

.. code-block:: shell-session

    python -m pip install --upgrade pip

Once pip finishes upgrading, rerun the installation command above to install Alpaca-py
